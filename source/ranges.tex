...
namespace std::ranges {
 ...
 template<@\libconcept{forward_range}@ V, @\libconcept{forward_range}@ Pattern>
  ...
  namespace views {
    inline constexpr @\unspec@ chunk = @\unspec@;
  }
  ...
  namespace views {
    inline constexpr @\unspec@ slide = @\unspec@;
  }
}
...
namespace std::ranges {
  ...
  public:
    ...
    constexpr V base() const & requires @\libconcept{copy_constructible}@<V> { return base_; }
...
namespace std::ranges {
  ...
  class chunk_view<V>::@\exposid{iterator}@ {
    ...
    iterator_t<@\exposid{Base}@> @\exposid{current_}@ = iterator_t<@\exposid{Base}@>();                     // \expos
    sentinel_t<Base> @\exposid{end_}@ = sentinel_t<@\exposid{Base}@>();                         // \expos
...
Equivalent to:
\begin{codeblock}
missing_ = ranges::advance(@\exposid{current_}@, @\exposid{n_}@, @\exposid{end_}@);
return *this;
\end{codeblock}
\end{itemdescr}
...
is \tcode{true}.
\begin{note}
If \tcode{x} is negative, the \effects paragraph implies a precondition.
\end{note}
...
\end{codeblock}
\item
Otherwise, \exposid{iterator}<false>(ranges::begin(\exposid{base_}), \exposid{n_}).
\end{itemize}
...
namespace std::ranges {
  ...
  class slide_view<V>::@\exposid{iterator}@ {
    ...
    constexpr @\exposid{iterator}@(iterator_t<@\exposid{Base}@> current, iterator_t<@\exposid{Base}@> @\exposid{last_ele}@,  // \expos
...
\pnum
\expects
\tcode{\exposid{current_} + x} and \tcode{\exposid{last_ele_} + x} (if present)
...
\pnum
\returns
x.\exposid{current_} < y.\exposid{current_}.
\end{itemdescr}
...
\rSec3[range.slide.sentinel]{Class slide_view::sentinel}
...
only when \exposconcept{slide-caches-first}<V> is \tcode{true}.
\end{note}
...
friend constexpr range_difference_t<V>
  operator-(const @\exposid{sentinel}@& y, const @\exposid{iterator}@<false>& x)
    requires \tcode{sized_sentinel_for}<sentinel_t<V>, iterator_t<V>>;
\end{itemdecl}
